...
public class Assignment_PatrolGuard : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        targetRotation = Quaternion.Euler(0f, startYAngle, 0f); // 초기 방향 설정
    ...
    private void Update()
    {
        ...
        quaternion turnRotation = Quaternion.FromToRotation(transform.forward, dirToNext);    // 두 방향 벡터 사이 회전 계산
        targetRotation = turnRotation * transform.rotation;   // 일정 각속도로 몸을 회전

        transform.rotation = Quaternion.RotateTowards(transform.rotation, targetRotation, turnSpeed * Time.deltaTime); // 회전 적용
