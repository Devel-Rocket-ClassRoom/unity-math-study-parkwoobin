...
public class Assignment_CircularFormation : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < units.Length; i++)
        {
            ...
            units[i].position = leader.position + leader.rotation * Quaternion.AngleAxis(angle, leader.up) * Vector3.forward * formationRadius;
            units[i].rotation = leader.rotation * Quaternion.Euler(0f, angle, 0f);
        }
        UpdateUI();
    }
