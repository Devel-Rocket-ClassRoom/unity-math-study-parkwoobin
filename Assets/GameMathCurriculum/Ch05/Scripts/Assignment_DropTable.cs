...
public class Assignment_DropTable : MonoBehaviour
{
    ...
    {
        totalWeight = 0f;
        for (int i = 0; i < items.Length; i++)  // 전체 가중치 합 계산
        {
            totalWeight += items[i].weight; // 가중치 합이 0이면 드롭 불가능
        }

        float randomValue = Random.value * totalWeight;
        float cumulativeWeight = 0f;    // 누적 가중치 계산

        for (int i = 0; i < items.Length; i++)  // 누적 확률 계산
        {
            cumulativeWeight += items[i].weight;
            if (randomValue <= cumulativeWeight)
            {
                lastSelectedIndex = i;  // 드롭된 아이템 인덱스 저장
                dropHistory.Add(i);  // 드롭 결과 기록
                if (dropHistory.Count > MAX_HISTORY)
                {
                    dropHistory.RemoveAt(0);  // 최대 기록 수 유지
                }
                break;  // 드롭 결과 결정 후 루프 종료
            }
        }
        UpdateUI();
    }
