...
public class Assignment_SplineConveyor : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        globalT += Time.deltaTime / cycleDuration;  // globalT는 0~1 범위를 반복하도록 한다.
        globalT = Mathf.Repeat(globalT, 1f);    // globalT에 따른 속도 배율 계산
        currentSpeedMultiplier = speedCurve.Evaluate(globalT);  // globalT에 따른 속도 배율 계산

        for (int i = 0; i < boxes.Length; i++)
        {
            boxes[i].position = EvaluateSpline(waypoints, Mathf.Repeat(currentSpeedMultiplier * globalT + (float)i / boxes.Length, 1f));
        }
